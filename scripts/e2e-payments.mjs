...
const base = (process.argv[2] || process.env.E2E_PAYMENTS_URL || 'https://api.connectivebyte.com').replace(
  /\/+$/,
  '',
);
...
async function main() {
  if (!process.env.DATABASE_URL) {
    console.error('❌ DATABASE_URL が設定されていません（role 昇格・grant/revoke・クリーンアップに必要）');
    process.exit(1);
  }
  ...
  const pub = await call(`/api/learning/sessions/${week1.sessions?.[0]?.slug ?? 'week-01-day-01'}`);
  ...
  assert('GET /payments/status（匿名）→ 401', anonStatus.res.status === 401, `HTTP ${anonStatus.res.status}`);

  const status0 = await call('/api/payments/status', { token });
  assert('GET /payments/status（未購入）→ 200', status0.res.status === 200, `HTTP ${status0.res.status}`);
  assert('purchased: false', status0.json?.data?.purchased === false, JSON.stringify(status0.json?.data));
  ...
  if (
    !assert('管理 module（week 2・公開）作成 → 201', newModule.res.status === 201, `HTTP ${newModule.res.status} ${JSON.stringify(newModule.json)}`)
  )
    return report();
  ...
  assert('管理 session（公開）作成 → 201', newSession.res.status === 201, `HTTP ${newSession.res.status} ${JSON.stringify(newSession.json)}`);
  ...
  await sql.query(`UPDATE users SET purchased_at = now(), updated_at = now() WHERE email = $1`, [email]);

  const status1 = await call('/api/payments/status', { token });
  assert('grant 後 status → 200・purchased: true', status1.json?.data?.purchased === true, JSON.stringify(status1.json?.data));
  assert('購入額が 29800 円', status1.json?.data?.purchase?.amountTotal === 29800, JSON.stringify(status1.json?.data?.purchase));

  const unlocked = await call(`/api/learning/sessions/${paidSlug}`, { token });
  assert('grant 後 有料 slug → 200', unlocked.res.status === 200, `HTTP ${unlocked.res.status} ${JSON.stringify(unlocked.json)}`);
  assert('有料本文が閲覧できる', (unlocked.json?.data?.session?.content ?? '').includes('有料本文'));
  ...
  assert('grant 後 PUT progress → 200', progressOk.res.status === 200, `HTTP ${progressOk.res.status} ${JSON.stringify(progressOk.json)}`);
  ...
  await sql.query(`UPDATE users SET purchased_at = NULL, updated_at = now() WHERE email = $1`, [email]);

  const status2 = await call('/api/payments/status', { token });
  assert('revoke 後 status → purchased: false', status2.json?.data?.purchased === false, JSON.stringify(status2.json?.data));
  ...
  const [purchaseLeft] = await sql.query('SELECT 1 FROM purchases WHERE stripe_checkout_session_id = $1', [
    checkoutSessionId,
  ]);
