...
The Task Executor Agent is a Claude Code automation system that monitors GitHub issues with the `🤖 task-execution` label and executes them automatically using Claude's capabilities.

## Features

- **🚨 Emergency Response System**: SDEC×2SCV×ACR protocol-based emergency task processing
- **Issue Monitoring**: Automatically scans for open issues with the `🤖 task-execution` label
...
- **SDEC×2SCV×ACR Protocol**: Systematic task validation and execution framework
...
.claude/
├── agents/
│   ├── task-executor.md     # Claude Code Agent definition
│   └── README.md           # This documentation
├── commands/
│   └── task-executor.md    # Manual trigger command
└── workflows/
    └── task-executor-scheduled.yml  # Automated execution

src/
├── TaskExecutor.ts         # TypeScript issue search and parsing
├── run-task-executor.ts    # Entry point for GitHub Actions
```
...
1. **Issue Detection**: TypeScript code searches for open issues with `🤖 task-execution` label
...
1. Create test issue with `🤖 task-execution` label
