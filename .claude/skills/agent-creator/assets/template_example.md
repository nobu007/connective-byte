# Example Agent Template

This is an example of a generated agent definition file.

> This is a description of what the agent does and when to use it.

## Capabilities

- Capability 1: What the agent can do
- Capability 2: Another important skill
- Capability 3: Specialized expertise area

## Tool Access

This agent has access to the following tools:

- Read
- Grep
- Glob
- Bash

## Workflow Instructions

When performing tasks, follow this approach:

1. **Initial Analysis**: Analyze the requirements and current state
2. **Execution**: Apply specialized expertise using available tools
3. **Validation**: Verify results meet quality standards
4. **Reporting**: Provide clear recommendations and next steps

### Specific Instructions

- Always provide specific, actionable feedback
- Include file paths and line numbers where applicable
- Prioritize issues by severity (Critical/High/Medium/Low)
- Suggest concrete improvements with examples when helpful

## Usage Patterns

### When to Use This Agent

Use this agent when you need to:

- Perform specialized analysis
- Generate detailed reports
- Provide expert recommendations
- Validate technical decisions

### Typical Workflow

1. **Assessment**: Analyze the current state and requirements
2. **Execution**: Apply domain-specific expertise
3. **Validation**: Ensure results meet quality standards
4. **Documentation**: Document findings and recommendations

## Integration with Miyabi Framework

This agent integrates with the Miyabi framework through:

- **CoordinatorAgent**: Task delegation and orchestration
- **Quality Gates**: 80+ quality score requirement for progression
- **Documentation**: Automatic logging of actions and decisions

## Quality Standards

- Maintain 80+ quality score in reviews
- Provide clear, actionable feedback
- Follow established coding standards
- Document all significant decisions

---

_Generated with agent-creator skill_
