 ...
 * ログインページ
 *
 * - メール/PW フォーム + Google ログイン（<a href> による top-level navigation）
 * - OAuth コールバックからの ?error=oauth_* を日本語メッセージへ変換
 * - ?next=<path> で認証後の遷移先を保持（RequireAuth から渡される）
 */
...
import { authApi, googleLoginUrl } from '@/lib/api/auth-api';
...
/** /login/?error= の OAuth エラーコード → 日本語メッセージ */
const OAUTH_ERROR_MESSAGES: Record<string, string> = {
  oauth_state: 'ログインセッションが無効です。もう一度お試しください。',
  oauth_failed: 'Googleログインに失敗しました。もう一度お試しください。',
  oauth_email_unverified: 'Googleアカウントのメールアドレスが未確認です。Google側で確認してから再度お試しください。',
  oauth_cancelled: 'ログインをキャンセルしました。',
  oauth_unavailable: 'Googleログインは現在利用できません。しばらくしてから再度お試しください。',
};
    ...
    <main className="pt-24 pb-16">
      <div className="max-w-md mx-auto px-4">
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-8">
          {oauthErrorMessage && (
            <div className="mb-6">
              <FormError message={oauthErrorMessage} />
            </div>
          )}
